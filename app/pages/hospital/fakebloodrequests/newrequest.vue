...
// ---------- MOCK DATA SWITCH ----------
// TODO: i-false ni sa dihang kompleto na ang backend endpoints
// (reference-data, blood-availability-summary, request-policy, requests, attachments).
const USE_MOCK_DATA = true;

// ---------- Fake/mock reference data ----------
...
const mockPhysicians = [
  { id: 1, name: 'Dr. Andrea Villamor' },
  { id: 2, name: 'Dr. Miguel Santos' },
  { id: 3, name: 'Dr. Liza Fernandez' },
  { id: 4, name: 'Dr. Renz Aguilar' },
  { id: 5, name: 'Dr. Patricia Cruz' },
];
...
const mockComponents = [
  { id: 1, name: 'Packed RBC' },
  { id: 2, name: 'Whole Blood' },
  { id: 3, name: 'Fresh Frozen Plasma' },
  { id: 4, name: 'Platelets' },
  { id: 5, name: 'Cryoprecipitate' },
];

const mockAvailability = [
  { blood_type: 'A+', units: 42, status: 'healthy' },
  { blood_type: 'A-', units: 9, status: 'low' },
  { blood_type: 'B+', units: 31, status: 'healthy' },
  { blood_type: 'B-', units: 4, status: 'critical' },
  { blood_type: 'AB+', units: 15, status: 'healthy' },
  { blood_type: 'AB-', units: 3, status: 'critical' },
  { blood_type: 'O+', units: 58, status: 'healthy' },
  { blood_type: 'O-', units: 7, status: 'low' },
];

const mockGuidelines = [
  'Submit requests at least 24 hours before the required date, except for emergency cases.',
  'Emergency and urgent priority requests are reviewed immediately by the Blood Center on-call staff.',
  'Attach supporting documents (lab requests, physician orders) to speed up approval.',
  'Double-check the blood type and component before submitting to avoid processing delays.',
  'You will be notified once your request status changes to Approved or Ready for Pickup.',
];

const mockProcessingTimeline = [
  { label: 'Request Submitted', duration: 'Immediate' },
  { label: 'Blood Center Review', duration: '1–4 hours' },
  { label: 'Approval & Cross-matching', duration: '2–6 hours' },
  { label: 'Preparation of Units', duration: '2–4 hours' },
  { label: 'Ready for Pickup', duration: 'Within 24 hours' },
];
...
onMounted(async () => {
  if (USE_MOCK_DATA) {
    // Gamay nga delay para ma-preview gihapon ang skeleton states
    await new Promise((resolve) => setTimeout(resolve, 400));
    bloodTypes.value = mockBloodTypes;
    components.value = mockComponents;
    departments.value = mockDepartments;
    physicians.value = mockPhysicians;
    loadingReference.value = false;

    await new Promise((resolve) => setTimeout(resolve, 300));
    availability.value = mockAvailability;
    loadingAvailability.value = false;

    guidelines.value = mockGuidelines;
    processingTimeline.value = mockProcessingTimeline;
    return;
  }
...
async function saveDraft() {
  ...
  if (USE_MOCK_DATA) {
    ...
    console.info('[mock] Draft saved locally:', { ...form });
...
function uploadOne(file, entry) {
  if (USE_MOCK_DATA) {
    // Gina-simulate ang upload progress para dunay tinuod nga rasa ang UI
    // samtang wala pa nay POST /hospital/requests/attachments endpoint.
    const timer = setInterval(() => {
      entry.progress = Math.min(100, entry.progress + Math.round(15 + Math.random() * 20));
...
async function submitRequest() {
  ...
  if (USE_MOCK_DATA) {
    ...
    const refNumber = `BR-2026-${Math.floor(1000 + Math.random() * 9000)}`;
