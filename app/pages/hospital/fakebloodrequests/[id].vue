...
// ---------- MOCK DATA SWITCH ----------
// TODO: i-false ni sa dihang kompleto na ang useBloodRequestDetails composable
// ug ang mga backend endpoints (request detail, history, blood availability, timeline).
const USE_MOCK_DATA = true

const {
  request: apiRequest,
  history: apiHistory,
  bloodAvailability: apiBloodAvailability,
  timeline: apiTimeline,
  progressPercent: apiProgressPercent,
  isLoadingRequest: apiIsLoadingRequest,
  isLoadingAvailability: apiIsLoadingAvailability,
...
const mockRequest = {
  id: requestId,
  reference_number: 'BR-2026-0142',
  status: 'Processing',
  priority: 'Urgent',
  hospital: 'Davao Regional Medical Center',
  department: 'Emergency Department',
  requesting_physician: 'Dr. Andrea Villamor',
  patient_reference_number: 'HN-0042',
  purpose: 'Emergency transfusion support following vehicular accident with significant blood loss.',
  notes: 'Patient stable but requires close monitoring; coordinate release with ER charge nurse.',
  blood_type: 'O+',
  blood_component: 'Whole Blood',
  units_requested: 3,
  compatibility: 'Confirmed — crossmatch completed',
  special_requirements: 'Irradiated blood product requested due to patient history.',
  clinical_indication: 'Acute blood loss anemia secondary to trauma, hemoglobin at 6.2 g/dL on admission.',
  diagnosis: 'Multiple blunt force trauma, hypovolemic shock (resolving).',
  additional_notes: 'Physician requests notification once units are ready for pickup.',
  request_date: '2026-07-24',
  required_date: '2026-07-26',
  estimated_completion: '2026-07-27',
  documents: [
    { id: 1, name: 'Physician_Order_Form.pdf', url: '#' },
    { id: 2, name: 'Lab_Crossmatch_Result.pdf', url: '#' },
  ],
}

const mockHistory = [
  { id: 1, date: '2026-07-24T08:15:00', activity: 'Request submitted', performed_by: 'Dr. Andrea Villamor', remarks: 'Submitted via hospital portal.' },
  { id: 2, date: '2026-07-24T09:02:00', activity: 'Request reviewed', performed_by: 'Blood Center Staff — J. Reyes', remarks: 'Verified patient details and required documents.' },
  { id: 3, date: '2026-07-24T10:40:00', activity: 'Request approved', performed_by: 'Blood Center Staff — J. Reyes', remarks: 'Approved for processing, matched against inventory.' },
  { id: 4, date: '2026-07-24T13:20:00', activity: 'Crossmatching started', performed_by: 'Lab Technician — M. Cruz', remarks: 'Sample received, crossmatch in progress.' },
  { id: 5, date: '2026-07-25T07:55:00', activity: 'Units being prepared', performed_by: 'Blood Center Staff — R. Uy', remarks: 'Preparing 3 units of whole blood for dispatch.' },
]

const mockBloodAvailability = [
  { blood_type: 'A+', units_available: 42 },
  { blood_type: 'A-', units_available: 9 },
  { blood_type: 'B+', units_available: 31 },
  { blood_type: 'B-', units_available: 4 },
  { blood_type: 'AB+', units_available: 15 },
  { blood_type: 'AB-', units_available: 3 },
  { blood_type: 'O+', units_available: 58 },
  { blood_type: 'O-', units_available: 7 },
]

const mockTimeline = [
  { step: 'submitted', label: 'Request Submitted', status: 'completed', timestamp: '2026-07-24T08:15:00' },
  { step: 'reviewed', label: 'Reviewed', status: 'completed', timestamp: '2026-07-24T09:02:00' },
  { step: 'approved', label: 'Approved', status: 'completed', timestamp: '2026-07-24T10:40:00' },
  { step: 'preparing', label: 'Preparing Blood Units', status: 'current', timestamp: '2026-07-25T07:55:00' },
  { step: 'ready', label: 'Ready for Pickup', status: 'upcoming', timestamp: null },
  { step: 'completed', label: 'Completed', status: 'upcoming', timestamp: null },
]

const mockProgressPercent = 65

// ---------- Local loading state para sa mock mode ----------
const mockIsLoadingRequest = ref(true)
const mockIsLoadingAvailability = ref(true)

// ---------- Exposed values nga gigamit sa template ----------
// Kung naa nay tinuod nga backend, i-false lang ang USE_MOCK_DATA para
// direkta na gikan sa composable ang datos.
const request = computed(() => (USE_MOCK_DATA ? mockRequest : apiRequest.value))
const history = computed(() => (USE_MOCK_DATA ? mockHistory : (apiHistory.value ?? [])))
const bloodAvailability = computed(() => (USE_MOCK_DATA ? mockBloodAvailability : (apiBloodAvailability.value ?? [])))
const timeline = computed(() => (USE_MOCK_DATA ? mockTimeline : (apiTimeline.value ?? [])))
const progressPercent = computed(() => (USE_MOCK_DATA ? mockProgressPercent : (apiProgressPercent.value ?? 0)))
const isLoadingRequest = computed(() => (USE_MOCK_DATA ? mockIsLoadingRequest.value : apiIsLoadingRequest.value))
const isLoadingAvailability = computed(() => (USE_MOCK_DATA ? mockIsLoadingAvailability.value : apiIsLoadingAvailability.value))
const requestError = computed(() => (USE_MOCK_DATA ? null : apiRequestError.value))

onMounted(() => {
  if (USE_MOCK_DATA) {
    // Gamay nga delay para ma-preview gihapon ang skeleton loading states
    setTimeout(() => { mockIsLoadingRequest.value = false }, 500)
    setTimeout(() => { mockIsLoadingAvailability.value = false }, 750)
    return
  }
  fetchRequest()
  fetchAvailability()
...
async function handleDownloadPdf() {
  ...
  if (USE_MOCK_DATA) {
    console.info('[mock] Would download PDF for', request.value.reference_number)
    return
  }
  try {
