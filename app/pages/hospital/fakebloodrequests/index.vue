...
// ---------- MOCK DATA SWITCH ----------
// TODO: i-false ni sa dihang naa nay real backend endpoint (GET /hospital/blood-requests)
// Samtang wala pa, gamiton ang mockRequests below para makita dayon ang UI nga naay laman.
...
// ---------- Fake/mock data helpers ----------
// Nag-generate og lain-laing dates (relative sa "today") para tinuod tan-awon
// ang trends (this week vs last week) ug ang date filters.
function daysAgo(n) {
  const d = new Date()
...
const mockHospitalNames = ['Davao Regional Medical Center', 'Southern Philippines Medical Center', 'San Pedro Hospital']
const mockRequesters = ['Dr. Andrea Villamor', 'Dr. Miguel Santos', 'Dr. Liza Fernandez', 'Dr. Renz Aguilar']

const mockRequests = [
  { id: 1, reference_number: 'BR-2026-0142', hospital_name: mockHospitalNames[0], blood_type: 'O+', component: 'Whole Blood', units: 3, priority: 'emergency', status: 'processing', request_date: daysAgo(1), requested_by: mockRequesters[0], reason: 'Emergency transfusion following vehicular accident with significant blood loss.' },
  { id: 2, reference_number: 'BR-2026-0141', hospital_name: mockHospitalNames[0], blood_type: 'A+', component: 'Packed RBC', units: 2, priority: 'urgent', status: 'approved', request_date: daysAgo(2), requested_by: mockRequesters[1], reason: 'Pre-surgical stock for scheduled cardiac bypass.' },
  { id: 3, reference_number: 'BR-2026-0140', hospital_name: mockHospitalNames[1], blood_type: 'B-', component: 'Fresh Frozen Plasma', units: 4, priority: 'normal', status: 'ready', request_date: daysAgo(3), requested_by: mockRequesters[2], reason: 'Plasma replacement for coagulopathy management.' },
  { id: 4, reference_number: 'BR-2026-0139', hospital_name: mockHospitalNames[0], blood_type: 'AB+', component: 'Platelets', units: 1, priority: 'urgent', status: 'pending', request_date: daysAgo(4), requested_by: mockRequesters[3], reason: 'Thrombocytopenia support prior to chemotherapy cycle.' },
  { id: 5, reference_number: 'BR-2026-0138', hospital_name: mockHospitalNames[2], blood_type: 'O-', component: 'Whole Blood', units: 5, priority: 'emergency', status: 'completed', request_date: daysAgo(6), requested_by: mockRequesters[0], reason: 'Postpartum hemorrhage requiring immediate transfusion.' },
  { id: 6, reference_number: 'BR-2026-0137', hospital_name: mockHospitalNames[1], blood_type: 'A-', component: 'Packed RBC', units: 2, priority: 'normal', status: 'completed', request_date: daysAgo(8), requested_by: mockRequesters[1], reason: 'Chronic anemia management for dialysis patient.' },
  { id: 7, reference_number: 'BR-2026-0136', hospital_name: mockHospitalNames[0], blood_type: 'B+', component: 'Cryoprecipitate', units: 3, priority: 'urgent', status: 'rejected', request_date: daysAgo(9), requested_by: mockRequesters[2], reason: 'Fibrinogen replacement — request rejected due to insufficient stock, redirected to partner facility.' },
  { id: 8, reference_number: 'BR-2026-0135', hospital_name: mockHospitalNames[2], blood_type: 'AB-', component: 'Whole Blood', units: 1, priority: 'normal', status: 'cancelled', request_date: daysAgo(11), requested_by: mockRequesters[3], reason: 'Patient discharged early; transfusion no longer required.' },
  { id: 9, reference_number: 'BR-2026-0134', hospital_name: mockHospitalNames[0], blood_type: 'O+', component: 'Platelets', units: 2, priority: 'urgent', status: 'completed', request_date: daysAgo(13), requested_by: mockRequesters[0], reason: 'Platelet support for dengue patient with low count.' },
  { id: 10, reference_number: 'BR-2026-0133', hospital_name: mockHospitalNames[1], blood_type: 'A+', component: 'Fresh Frozen Plasma', units: 3, priority: 'normal', status: 'completed', request_date: daysAgo(15), requested_by: mockRequesters[1], reason: 'Liver disease-related coagulopathy correction.' },
  { id: 11, reference_number: 'BR-2026-0132', hospital_name: mockHospitalNames[0], blood_type: 'B+', component: 'Packed RBC', units: 4, priority: 'emergency', status: 'completed', request_date: daysAgo(17), requested_by: mockRequesters[2], reason: 'Trauma resuscitation, multiple gunshot wounds.' },
  { id: 12, reference_number: 'BR-2026-0131', hospital_name: mockHospitalNames[2], blood_type: 'O-', component: 'Whole Blood', units: 2, priority: 'urgent', status: 'completed', request_date: daysAgo(20), requested_by: mockRequesters[3], reason: 'Emergency surgical bleed control.' },
]
...
async function downloadPdf(req) {
  closeMenu()
  if (USE_MOCK_DATA) {
    console.info('[mock] Would download PDF for', req.reference_number)
    return
  }
  try {
...
async function cancelRequest(req) {
  ...
  if (USE_MOCK_DATA) return // walay balik-balik nga API call kung mock pa
  try {
...
async function loadRequests() {
  ...
  if (USE_MOCK_DATA) {
    // Gamit gamay nga delay para ma-preview gihapon ang skeleton loading state
    await new Promise((resolve) => setTimeout(resolve, 500))
    allRequests.value = mockRequests.map(normalizeRequest)
    loading.value = false
